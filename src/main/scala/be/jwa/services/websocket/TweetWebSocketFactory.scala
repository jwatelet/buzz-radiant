...
trait TweetWebSocketFactory {
  ...
  private def createTweetWebsocketHandler(observerId: UUID): WsHandler = {
    val source: Source[Message, ActorRef] =
      Source.actorRef(bufferSize = 1, overflowStrategy = OverflowStrategy.dropHead)
    ...
    val (streamEntry: ActorRef, messageSource: Source[Message, NotUsed]) = source.toMat(BroadcastHub.sink(1))(Keep.both).run
